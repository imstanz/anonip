...
# Ensure ran as root
if [ $(id -u) -ne 0 ]; then
	echo "Are you Stoned? This script MUST be ran as root"
	exit 1
fi
...
then
	...
	apt-get install curl libecj-java libgetopt-java libservlet3.0-java glassfish-javaee ttf-dejavu i2p i2p-router libjbigi-jni #installs i2p and other dependencies
...
dpkg -i anonip.deb || (apt-get -f install && dpkg -i anonip.deb) # automatically install required packages
