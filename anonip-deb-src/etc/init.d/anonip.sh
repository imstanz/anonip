...
# Anonip is inspired by the homonimous module of PenMode.
# Developed by the "Pirates' Crew" of "ParrotSec", it is part of # parrot-anon package.
# Devs:
# Lorenzo 'EclipseSpark' Faletra <eclipse@frozenbox.org>
# Lisetta 'Sheireen' Ferrero <sheireen@frozenbox.org>
# Francesco 'mibofra'/'Eli Aran'/'SimpleSmibs' Bonanno <mibofra@frozenbox.org>
#
# anonip & anonsurf is free software: you can redistribute it and/or
# modify it under the terms of the GNU General Public License as
# published by the Free Software Foundation, either version 3 of the
# License, or (at your option) any later version.
# You can get a copy of the license at www.gnu.org/licenses
#
# anonip & anonsurf is distributed in the hope that it will be
# useful, but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
# General Public License for more details.
# You should have received a copy of the GNU General Public License along with
# Parrot Security OS & the below links. If not, see <http://www.gnu.org/licenses/>.
#
# Cloned & hacked, from: https://github.com/Und3rf10w/kali-anonsurf, by
# 'stanz' @ https://git.devuan.org/stanz/anonip
# stanz added palemoon: line 65, removed Parrot - added stanz: line 302
#
...
function init {
	echo -e -n " $GREEN*$BLUE killing some dangerous apps$RESETCOLOR\n"
	killall -q chrome dropbox iceweasel palemoon skype icedove thunderbird firefox chromium xchat transmission deluge pidgin pidgin.orig
...
function start {
	...
	sed -i '/^.*\#anonip$/d' /etc/sysctl.conf #delete lines containing #anonip in /etc/sysctl.conf
	# add lines to sysctl.conf that will kill ipv6 services
	echo "net.ipv6.conf.all.disable_ipv6 = 1 #anonip" >> /etc/sysctl.conf
	echo "net.ipv6.conf.default.disable_ipv6=1 #anonip" >> /etc/sysctl.conf
	...
	echo -e "$GREEN[$BLUE i$GREEN ]$BLUE You are under Anonip tunnel$RESETCOLOR\n"
}
...
function stop {
	...
	echo -e " $GREEN*$BLUE Anonymous mode stopped, get dressed!$RESETCOLOR\n"
}
...
case "$1" in
   ...
   *)
echo -e "
stanzHack anonip Module
	Usage:
	$RED┌──[$GREEN$USER$YELLOW@$BLUE`hostname`$RED]─[$GREEN$PWD$RED]
	$RED└──╼ \$$GREEN"" anonip $RED{$GREEN""start$RED|$GREEN""stop$RED|$GREEN""restart$RED|$GREEN""change$RED""$RED|$GREEN""status$RED""}
	...
	$RED status$BLUE -$GREEN Check if anonip is working properly
